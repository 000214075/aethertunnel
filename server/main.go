...
func main() {
    ...
    if len(os.Args) < 2 {
        ...
        fmt.Println("\nConfig file example:")
        fmt.Println(server_simple_example)
        os.Exit(1)
    }
    ...
    if cfg.Dashboard.Enabled {
        go func() {
            if err := StartDashboard(cfg.Dashboard.Port, cfg); err != nil {
    ...
    // 创建代理管理器
    proxyManager := NewProxyManager(cfg, encryption)
...
// server_simple_example 服务端简单配置示例
const server_simple_example = `[server]
