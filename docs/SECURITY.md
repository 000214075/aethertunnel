...
窗口内再犯才按倍数增长，窗口过后它和从未失败过的地址一样被忘掉。因此**负载均衡、健康检查与
监控地址必须写进 `ban_ignore_cidrs`**，否则它们会和攻击者共用同一个来源地址。
...
- 只需证明**某一条**的包含性时，用
  `aethertunnel-server --ledger-proof <文件> --proof-index <n>` 导出到第 n 条为止的前缀：
  这段前缀用同一把公钥单独可校验，链头就是整条链在第 n 条的哈希，所以不必把整条链交出去。
